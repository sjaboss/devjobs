...
{{-- <section class="bg-white dark:bg-gray-900">
    <div class="py-8 px-4 mx-auto max-w-screen-xl lg:py-16">
        ...
        <div class="grid md:grid-cols-2 gap-8">
            ...
            <div class=" bg-white shadow-sm rounded-lg p-6 mt-3 divide-y divide-gray-200">
                @forelse ($vacantes as $item)
               {{--      <div class="md:justify-between md:items-center  md:grid md:grid-cols-3 "> --}}
                <div class="bg-gray-800 border border-gray-700 rounded-lg p-4 md:p-6 mb-4 md:justify-between md:items-center  md:grid md:grid-cols-3">
                        <img class="rounded-l-lg shadow-lg" src="{{ asset('storage/public/vacantes/' . $item->imagen) }}">

                        <div class="md:flex-1 pl-5">
                            <a
                                class="text-2xl font-extrabold text-white mb-5   "href="{{ route('vacantes.show', $item->id) }}">{{ $item->titulo }}</a>
                            <p class="mt-1 text-base font-bold text-gray-400 mb-3">Empresa: <samp class="font-normal">
                                    {{ $item->empresa }} </samp> </p>
                            <p class="text-base font-bold text-gray-400 mb-3">Categoría: <span class="font-normal">
                                    {{ $item->categoria->categoria }} </span></p>
                            <p class="text-base font-bold text-gray-400 mb-3">Salario: <span class="font-normal">
                                    {{ $item->salario->salario }} </span> </p>
                            <p class="font-bold mb-2 text-xs text-gray-400">Último día para postularse: <span
                                    class="font-normal">{{ date('d/m/Y', strtotime($item->ultimo_dia)) }}</span> </p>
                        </div>


                        <div class="mt-5 md:mt-0">
                            <a href="{{ route('vacantes.show', $item->id) }}"
                                class="bg-slate-800 py-2  px-4 
                                 rounded-l-lg border-b-4 border-l-8 border-t-4  rounded-r-lg border-r-8 border-gray-400   hover:border-lime-500 text-white text-xs font-bold uppercase  block text-center">Ver
                                Vacantes</a>
                        </div>
                    </div>
